...
<head>
  ...
  <style>
    ...
    .timeline-card::before {
      ...
      left: -2px;
      width: 4px;
      background: linear-gradient(to bottom, #00ff66 0%, #00ff66 100%);
      border-radius: 2px;
    }
...
<body x-data="{ explore: false }">
  ...
  <div class="wrap">
    <div class="shell">
      ...
      <header>
        <nav class="navbar navbar-expand-lg navbar-dark w-100" >
          <div class="container-fluid">
            <a class="navbar-brand fw-bold text-white" href="#">{{$firstname}} {{$secondname}}</a>
      ...
      <main class="hero">
        ...
        <section class="left">
          <h1 class="name"><span class="accent">{{$firstname}}</span>{{$secondname}}</h1>
            ...
            <a class="download" title="Download CV">
              <span class="icon" aria-hidden="true">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                  ...
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" stroke="currentColor" stroke-width="1.6"
        ...
        <aside class="right" aria-hidden="false">
          <div class="artboard" role="img" aria-label="Profile artwork area">
            ...
            <img class="profile-img" src="{{ $profileImage ?? asset($image) }}" alt="Profile photo">
          </div>
        </aside>
      ...
      <div x-show="explore" x-cloak x-transition
        style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:1500;">
  ...
  <div id="about">@include('template_1.components.prof', ['summary'=>$summary,'twitter'=>$twitter,'linkedin'=>$linkedin,'github'=>$github])</div>
  <div id="skills">@include('template_1.components.skills', ['skills' => $skills])</div>
  <div id="education">@include('template_1.components.education', ['education' => $education])</div>
  <div id="projects">@include('template_1.components.projects', ['projects' => $projects])</div>
  <div id="experience">@include('template_1.components.experience', ['experiences' => $experiences])</div>
  <div>@include('template_1.components.volunteer', ['volunteers' => $volunteers])</div>
  <div>@include('template_1.components.publications', ['publications' => $publications])</div>
  <div>@include('template_1.components.awards', ['awards' => $awards])</div>
  <div>@include('template_1.components.hobbies', ['hobbies' => $hobbies])</div>
  <div>@include('template_1.components.references', ['references' => $references])</div>
  <div>@include('template_1.components.contact')</div>
